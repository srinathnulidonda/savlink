...
def _parse_expiration(raw: str) -> Tuple[Optional[datetime], Optional[str]]:
    ...
    deltas = {'h': timedelta(hours=n), 'd': timedelta(days=n), 'w': timedelta(weeks=n),
...
def _append_utm(url: str, params: Dict[str, str]) -> str:
    ...
    mapping = {'source': 'utm_source', 'medium': 'utm_medium', 'campaign': 'utm_campaign',
...
def check_duplicate(user_id: str, url: str) -> Optional[Dict]:
    ...
    existing = Link.query.filter(
        ...
        (Link.original_url == normalized) | (Link.original_url == normalized + '/') | (Link.original_url == url)
    ).first()
    if not existing:
        return None
    return {'exists': True, 'existing_link_id': existing.id, 'existing_title': existing.title,
